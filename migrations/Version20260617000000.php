...
final class Version20260617000000 extends AbstractMultiPlatformMigration
{
    ...
    {
        $this->addSql('CREATE TABLE orders (id INT GENERATED BY DEFAULT AS IDENTITY NOT NULL, name VARCHAR(255) NOT NULL, notes TEXT NOT NULL, last_modified TIMESTAMP(0) WITHOUT TIME ZONE DEFAULT CURRENT_TIMESTAMP NOT NULL, datetime_added TIMESTAMP(0) WITHOUT TIME ZONE DEFAULT CURRENT_TIMESTAMP NOT NULL, PRIMARY KEY(id))');
        $this->addSql('CREATE TABLE order_items (id INT GENERATED BY DEFAULT AS IDENTITY NOT NULL, order_id INT NOT NULL, part_id INT DEFAULT NULL, supplier_id INT DEFAULT NULL, name VARCHAR(255) NOT NULL, quantity DOUBLE PRECISION NOT NULL, supplier_part_nr VARCHAR(255) DEFAULT NULL, last_modified TIMESTAMP(0) WITHOUT TIME ZONE DEFAULT CURRENT_TIMESTAMP NOT NULL, datetime_added TIMESTAMP(0) WITHOUT TIME ZONE DEFAULT CURRENT_TIMESTAMP NOT NULL, PRIMARY KEY(id))');
        $this->addSql('CREATE INDEX IDX_62809DB08D9F6D38 ON order_items (order_id)');
        $this->addSql('CREATE INDEX IDX_62809DB04CE34BEC ON order_items (part_id)');
        $this->addSql('CREATE INDEX IDX_62809DB02ADD6D8C ON order_items (supplier_id)');
        $this->addSql('CREATE TABLE order_supplier_references (id INT GENERATED BY DEFAULT AS IDENTITY NOT NULL, order_id INT NOT NULL, supplier_id INT DEFAULT NULL, order_number VARCHAR(255) NOT NULL, last_modified TIMESTAMP(0) WITHOUT TIME ZONE DEFAULT CURRENT_TIMESTAMP NOT NULL, datetime_added TIMESTAMP(0) WITHOUT TIME ZONE DEFAULT CURRENT_TIMESTAMP NOT NULL, PRIMARY KEY(id))');
        $this->addSql('CREATE INDEX IDX_31A9D8078D9F6D38 ON order_supplier_references (order_id)');
        $this->addSql('CREATE INDEX IDX_31A9D8072ADD6D8C ON order_supplier_references (supplier_id)');
